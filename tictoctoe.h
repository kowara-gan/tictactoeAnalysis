...
struct State{
  ...
  //Generate state from the 11-digit notation generated by getStateNum()
  ...
  uint64 getNormalizeBit() const{
    if(turn==FIRST){
      State a = *this;
      uint64 u1=getStateBit();
      a.flipVertically();
      u1=std::min(u1,a.getStateBit());
      a.flipDiagonally();
      u1=std::min(u1,a.getStateBit());
      a.flipVertically();
      u1=std::min(u1,a.getStateBit());
  ...
  vUint64 nextNormalizeBits() const{
    vUint64 ret;
    ...
    for(int x=0;x<3;x++)
      for(int y=0;y<3;y++){
          if(board[x*3+y]==0){
              ...
              s.stands[0]--;
              s.changeTurn();
              ret.push_back(s.getNormalizeBit());
          }
      }
    return ret;
  }
  //Generate all of the next position by getStateNum()
  vUint64 nextStateNums() const{
    vUint64 ret;
    if(stands[0]==0)return ret;
    for(int x=0;x<3;x++)
      for(int y=0;y<3;y++){
          if(board[x*3+y]==0){
              ...
              s.stands[0]--;
              s.changeTurn();
              ret.push_back(s.rotateChangeTurnState().getStateNum());
          }
      }
    return ret;
  }
  ...
  //Judge first turn form stands
  bool isFirst() const{
    return (stands[0]+stands[1])%2!=0;
  }
};
#endif